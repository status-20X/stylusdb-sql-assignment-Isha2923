...
const {parseQuery, parseQueryWhere ,parseQuerySix,parseQueryEight,parseQueryNine,parseQueryTen}= require('../../src/queryParser');
const {executeSELECTQuery,executeSELECTQueryfour,executeSELECTQuerywhere,executeSELECTQuerySix,executeSELECTQuerySeven,executeSELECTQueryEight,executeSELECTQueryNine,executeSELECTQueryTen} = require('../../src/index');
...
test('Execute COUNT Aggregate Query', async () => {
    ...
    const result = await executeSELECTQueryTen(query);
...
test('Parse LEFT JOIN clause', () => {
    ...
    const result = parseQueryNine(query);
    expect(result).toEqual({
        joinType: 'LEFT',
        joinTable: 'table2',
...
test('Returns null for queries without JOIN', () => {
    ...
    const result = parseQueryTen(query);
    expect(result).toEqual(
        {
            joinType: null,
            joinTable: null,
...
test('Parse SQL Query with LEFT JOIN with a WHERE clause filtering the main table', async () => {
    const query = 'SELECT student.name, enrollment.course FROM student LEFT JOIN enrollment ON student.id=enrollment.student_id WHERE student.age > 22';
    const result = await parseQueryTen(query);
    expect(result).toEqual({
        "fields": ["student.name", "enrollment.course"],
        "joinCondition": { "left": "student.id", "right": "enrollment.student_id" },
        "joinTable": "enrollment",
...
test('Parse COUNT Aggregate Query', () => {
    const query = 'SELECT COUNT(*) FROM student';
    const parsed = parseQueryTen(query);
